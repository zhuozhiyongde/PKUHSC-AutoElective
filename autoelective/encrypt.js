function strEnc(r, e, a, n) {
    ...
    if (
        ...
        null != n && '' != n && ((o = getKeyBytes(n)), (m = o.length)),
        i > 0)
    )
        if (i < 4) {
            var l = strToBt(r);
            if (null != e && '' != e && null != a && '' != a && null != n && '' != n) {
                for (j = l, b = 0; b < c; b++) j = enc(j, t[b]);
                for (h = 0; h < f; h++) j = enc(j, s[h]);
                for (x = 0; x < m; x++) j = enc(j, o[x]);
                A = j;
...
function initPermute(r) {
    ...
    for (i = 0, m = 1, n = 0; i < 4; i++, m += 2, n += 2)
        for (j = 7, k = 0; j >= 0; j--, k++) (e[8 * i + k] = r[8 * j + m]), (e[8 * i + k + 32] = r[8 * j + n]);
    return e;
}
...
function generateKeys(r) {
    ...
    for (t = 0; t < 7; t++) for (j = 0, k = 7; j < 8; j++, k--) e[8 * t + j] = r[8 * k + t];
    var t = 0;
    for (t = 0; t < 16; t++) {
        var s = 0,
            o = 0;
